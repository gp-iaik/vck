...
- Trusted relying parties:
    - Add `RelyingPartyTrust`, configuring how a wallet establishes trust in the relying party sending an authorization request, per client identifier scheme, and a `relyingPartyTrust` parameter on `OpenId4VpHolder` and on `OpenId4VpWallet`
